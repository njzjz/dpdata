...
class TestPOSCARDump(unittest.TestCase, TestPOSCARoh):
    ...
    def setUp(self):
        ...
        tmp_system.from_lammps_lmp(os.path.join(
            'poscars', 'conf.lmp'), type_map=['O', 'H'])
...
class TestPOSCARSkipZeroAtomNumb(unittest.TestCase):
    ...
    def test_dump_vasp_type_map(self):
        system0 = dpdata.System(os.path.join(
            'poscars', 'POSCAR.oh.d'), fmt='vasp/poscar', type_map=['H', 'O'])
        system0.to_vasp_poscar('POSCAR.tmp.1')
        system1 = dpdata.System(os.path.join(
            'poscars', 'POSCAR.oh.d'), fmt='vasp/poscar', type_map=['C', 'H', 'A', 'O', 'B'])
