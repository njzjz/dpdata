...
def dump(folder,
    ...
    if len(sets) > 0:
        if remove_sets:
            ...
            raise RuntimeError('found ' + str(sets) + ' in ' + folder +
    ...
    for ii in range(nsets):
        ...
        np.save(os.path.join(set_folder, 'coord'),
                coords[set_stt:set_end])
        if eners is not None:
            np.save(os.path.join(set_folder, 'energy'), eners[set_stt:set_end])
        if forces is not None:
            np.save(os.path.join(set_folder, 'force'),
                    forces[set_stt:set_end])
